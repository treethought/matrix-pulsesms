	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(auth) == 0 && strings.HasSuffix(r.URL.Path, "/login") {
			...
			for _, part := range authParts {
				...
				if strings.HasPrefix(part, "dev.spherics.pulsesms.auth-") {
					auth = part[len("dev.spherics.pulsesms.auth-"):]
					break
				}
			}
...
func (prov *ProvisioningAPI) DeleteSession(w http.ResponseWriter, r *http.Request) {
	...
	if user.Session == nil && user.Pulse == nil {
...
func (prov *ProvisioningAPI) DeleteConnection(w http.ResponseWriter, r *http.Request) {
	...
	if user.Pulse == nil {
		jsonResponse(w, http.StatusNotFound, Error{
			Error:   "You don't have a PulseSMS connection.",
			ErrCode: "not connected",
		})
		return
	}
	user.DeleteConnection()
	jsonResponse(w, http.StatusOK, Response{true, "Disconnected from PulseSMS and connection deleted"})
}
...
func (prov *ProvisioningAPI) Disconnect(w http.ResponseWriter, r *http.Request) {
	...
	jsonResponse(w, http.StatusOK, Response{true, "Disconnected from PulseSMS"})
}
...
// 		} else {
// 			user.Connect(false)
// 			jsonResponse(w, http.StatusOK, Response{true, "Created connection to PulseSMS."})
// 		}
// 		return
// 	}
...
func (prov *ProvisioningAPI) Ping(w http.ResponseWriter, r *http.Request) {
	...
	loggedIn := user.Session != nil

	if user.Pulse != nil {
		wa["conn"] = map[string]interface{}{
			"is_connected": user.Pulse.IsConnected(),
			"is_logged_in": loggedIn,
		}
		// user.log.Debugln("Pinging PulseSMS mobile due to /ping API request")
		...
		// 	"err": errStr,
		// }
		user.log.Debugfln("Admin test response for /ping: (conn: %t, login: %t)",
			user.Pulse.IsConnected(), loggedIn)
	}
...
var upgrader = websocket.Upgrader{
	...
	Subprotocols: []string{"dev.spherics.pulsesms.login"},
}
...
// 	session, pid, err := user.Conn.Login(qrChan, ctx)
// 	qrChan <- "stop"
// 	if err != nil {
// 		var msg string
...
// 		return
// 	}
// 	user.log.Debugln("Successful login as", pid, "via provisioning API")
// 	user.ConnectionErrors = 0
